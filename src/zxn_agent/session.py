...
def _state_data(st: State) -> dict[str, Any]:
    return {
        ...
        "task_cache_hit_tok": st.task_cache_hit_tok,
        "task_cache_miss_tok": st.task_cache_miss_tok,
        "task_reasoning_tok": st.task_reasoning_tok,
        "task_cache_usage_reported": st.task_cache_usage_reported,
        "task_reasoning_usage_reported": st.task_reasoning_usage_reported,
...
def restore_state(data: dict[str, Any], session_id: str) -> State:
    ...
    return State(
        ...
        task_cache_hit_tok=max(0, int(data.get("task_cache_hit_tok", 0))),
        task_cache_miss_tok=max(0, int(data.get("task_cache_miss_tok", 0))),
        task_reasoning_tok=max(0, int(data.get("task_reasoning_tok", 0))),
        task_cache_usage_reported=bool(data.get("task_cache_usage_reported", False)),
        task_reasoning_usage_reported=bool(
            data.get("task_reasoning_usage_reported", False)
        ),
